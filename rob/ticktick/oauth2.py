...
class OAuth2:
    ...
    def _open_auth_url_in_browser(self):
        ...
        """
        log.info(
            "Providing authentication requires interacting with the web browser. Once you accept the "
    ...
    def get_access_token(self, check_cache: bool = True, check_env: str = None):
        ...
        if check_env is not None:
            ...
            try:
                ...
                raise ValueError(
                    "Access token in the environment must be a python dictionary contained"
                    " in a string literal"
                )
    ...
    def _set_expire_time(token_dict):
        ...
        token_dict["readable_expire_time"] = time.asctime(
            time.localtime(time.time() + token_dict["expires_in"])
        )
        return token_dict
