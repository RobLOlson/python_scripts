...
#--- Special Methods <Frame> -----------------------------------------------
    ...
    def __repr__(self):
        return "Frame({}, {}, {}, {})".format(self.width, self.height,
                                              self.padding, self.frames)
...
"""
    ...
    def topFrame(self, value):
        ...
        if self.topFrame and self.botFrame and self.leftFrame and self.rightFrame:
            self._frames = True
        elif (not self.topFrame) and (not self.botFrame) and (not self.leftFrame) and (not self.rightFrame):
            self._frames = False
        else:
            self._frames = None
...
#--- self._parallelize(...) <Frame private method> --------------------
    ...
    def _parallelize(self, texts=[], widths = None, **kwargs):
        ...
        args = [(frame(elem,width,padding=-1) if width != -1 else elem) for elem, width in zip(args,widths)]
...
#
    ...
    def _columnize(self, texts=[], width=None, height=None, padding=None,
                   frames=None, hJust = None, vJust = None, **kwargs):
...
#
    ...
    def _apply(self, text, width=None, height=None, padding=None,
          background_Char = None, hJust = None, vJust = None,
          topFrame=None, botFrame=None,
          leftFrame=None, rightFrame=None):
    ...
    ##            width = 80
        ...
        text = self._justify(text, canvasWidth,
                       canvasHeight, hJust, vJust)
        ...
        if minimumHeight > height:
            dud = ("+" if leftFrame else "") +\
                  "-"*(width-leftFrame-rightFrame) +\
                  ("+" if rightFrame else "")

            pdud = ("|" if leftFrame else "") + (_+_)*padding +\
                  _*(canvasWidth) +\
                  (_+_)*padding + ("|" if rightFrame else "")
            ...
            if height == 1:
                if topFrame:
                    ...
                    dud = (("|" if leftFrame else "") + (_+_)*padding +\
            ...
            count = 0
            while count < diff:
                mid.append(("|" if leftFrame else "") + (_+_)*padding +\
        ...
        lines = [("|" if leftFrame else "") +\
                 (_+_)*padding + line + (_+_)*padding +\
                 ("|" if rightFrame else "") for line in lines]
    ...
    def _justify(text, width= -1, height = -1, hJust = "left", vJust = "top"):
        ...
        if len(justifiedLines) < height:
            ...
            if vJust == "middle":
                ...
                justifiedLines = ['']*(heightDiff//2)+justifiedLines+['']*(heightDiff//2+heightDiff%2)
            if vJust == "bottom":
...
#--- @height.setter <Panel property> -----------------------------------------
    ...
    def height(self, value):
        ...
        if self._sisters:
            if value > self._parent._height - self._parent._padding*2 -\
               self._parent._topFrame - self._parent._botFrame - sistersHeight:
                raise PanelError("Child Panels cannot exceed their parent's height.")
            
        if dynamicSisters:
            dynamicSisters = sorted(dynamicSisters,
                                    key = lambda x: x._height)
            ...
            if biggestHeight <= 0:
                ...
                increments=[(elem._height//biggestHeight,
                             0,
                             elem.tname) for elem in dynamicSisters]
            
            deltaHeight = abs(value - self._height)

            while deltaHeight > 0:
                while not any([count > 1 for (inc, count, tname) in increments]):
                    increments = [(inc, count+inc, tname) for (inc, count, tname) in increments]
        ...
        dynamicHeight = self._height - self._contentLines - sum(elem._contentLines for elem in staticDaughters)
    ...
    def width(self, value):
        ...
        if self._parent:
            if self._parent._vertical:
                self._width = self._parent._width - self._parent._padding*4 -\
                              self._parent._leftFrame - self._parent._rightFrame
                return
        ...
        if self._sisters:
            if value > self._parent._width - self._parent._padding*4 -\
               self._parent._leftFrame - self._parent._rightFrame - sistersWidth:
                raise PanelError("Child Panels cannot exceed their parent's width.")
            
        if dynamicSisters:
            dynamicSisters = sorted(dynamicSisters,
                                    key = lambda x: x._width)
            ...
            if biggestWidth <= 0:
                ...
                increments=[(elem._width/biggestWidth,
                             0,
                             elem.name) for elem in dynamicSisters]
            
            deltaWidth = math.abs(value - self._width)

            while deltaWidth > 0:
                while not any([count > 1 for (inc, count, name) in increments]):
                    increments = [(inc, count+inc, name) for (inc, count, name) in increments]
...
NOTE: Previous daughters are abandoned."""
        ...
        for count in range(pieces):
            ...
            if self._vertical:
                width = self._width-self.leftFrame-self.rightFrame-self.padding*2
                if heights:
                    height = heights[count]
                else:
                    height = (self._height-self.topFrame-self.botFrame-self.padding*4)//pieces
            else:
                height = self._height-self.topFrame-self.botFrame-self.padding*4
                if widths:
                    width = widths[count]
                else:
                    width = (self._width-self.leftFrame-self.rightFrame-self.padding*4)//pieces
    ...
    def render(self, *args, **kwargs):
        ...
        if not args and not kwargs:
            ...
            if self._horizontal:
                ...
                return frame(canvas[:-1]+'\n'+columnize([elem.render() for elem in self._daughters], frames=False),
                             width=self.width,
                             height=self.height,
                             padding=self.padding,
                             topFrame=self.topFrame,
...
def frame(text, width=-1, height=-1, padding=0,
          background_Char=" ", hJust="left", vJust="top",
